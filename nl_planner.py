...
def parse_question(question, df):
    ...
    # WHY \w INSTEAD OF a-zA-Z0-9_:
    # \w in Python already matches accented/unicode letters (é, ñ, etc.)
    # by default, not just plain ASCII. The old pattern silently failed to
    # match phrases like "by région" — found during testing.
    group_match = re.search(r"\b(?:by|per)\s+([\w ]+?)(?:\?|$)", text)
...
def _find_agg_keyword(text):
    # WHY \b (word boundary) INSTEAD OF PLAIN SUBSTRING MATCHING:
    # A plain "in text" check would match "count" inside the word
    # "countries" — a real bug caught during testing. Word boundaries (\b)
    # make sure "count" only matches the whole word "count", not a
    # fragment buried inside an unrelated word.
...
def _normalise_to_words(text):
    """
    Breaks a piece of text into individual lowercase words, ignoring
    punctuation and anything inside brackets/parentheses (like units, e.g.
    "Sales (USD)" -> just "sales"). Shared by _find_column below for
    matching both the question text and real column names the same way.
    """
    text = re.sub(r"[\(\[].*?[\)\]]", " ", str(text))  # drop bracketed content
...
def _find_column(candidate_text, columns):
    """
    Matches a chunk of free text (e.g. "stores", "région", "sales usd")
    against the dataset's real column names, by comparing individual
    words rather than the whole phrase at once. This handles two things
    found during testing:
    1. A short column name like "Age" accidentally matching inside an
       unrelated word like "average" — fixed by comparing whole words,
       never substrings.
    2. A real column name with extra decoration, like "Café Région (€)",
       failing to match a plain word like "région" — fixed by stripping
       bracketed content/punctuation from both sides before comparing.
    Simple plurals (e.g. "stores" -> "Store") are handled by also trying
    each word with a trailing 's' removed.
    """
    candidate_words = set(_normalise_to_words(candidate_text))
    candidate_words |= {w.rstrip("s") for w in candidate_words if w.rstrip("s")}

    best_match = None
    best_score = 0
    for column in columns:
        column_words = set(_normalise_to_words(column))
        column_words |= {w.rstrip("s") for w in column_words if w.rstrip("s")}
        overlap = candidate_words & column_words
        if overlap:
            # Prefer the match backed by the longest shared word, since a
            # longer shared word is a more specific, less coincidental match.
            score = max(len(w) for w in overlap)
            if score > best_score:
                best_score = score
                best_match = column
    return best_match
...
def _plan_top_n(text, n, df, columns, numeric_columns, question):
    ...
    grouped_match = re.search(r"top \d+\s+([\w ]+?)\s+by\s+([\w ]+)", text)
    if grouped_match:
