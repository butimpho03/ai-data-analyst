...
def _find_column(candidate_text, columns):
    ...
    the dataset's real column names. Uses word-boundary matching (not
    plain substring matching) so a short column name like "Age" doesn't
    accidentally match inside an unrelated word like "average" — a real
    bug caught during testing. Handles simple plurals (e.g. "stores" ->
    "Store") by also trying the singular form.
    """
    ...
    for column in columns:
        ...
        for variant in {col_lower, col_singular}:
            if not variant:
                continue
            # WHY "s?" IN THE PATTERN:
            # This lets the matched word in the TEXT optionally end in an
            # extra "s" too (e.g. column "Product" matching the word
            # "products" in the question), on top of already trying the
            # column name's own singular form above. Covers plurals in
            # both directions without falling back to unsafe substring
            # matching.
            pattern = r"\b" + re.escape(variant) + r"s?\b"
            if re.search(pattern, candidate_text):
                if best_match is None or len(variant) > len(str(best_match).lower()):
                    best_match = column
                break
    return best_match
