"""
AI Data Analyst Assistant — Phase 7: AI provider integration.

WHAT THIS FILE IS:
The single entry point Streamlit runs. As of Phase 7, if a Groq API key is
configured, an "Explain this with AI" button appears after each result,
turning the already-calculated number/table into a plain-English
explanation. The AI never calculates anything itself — pandas already did
that in analysis_engine.py. If no key is configured, or the AI call fails
for any reason, the app keeps working exactly as before, just without the
extra explanation.
"""
    ...
    if st.button("Analyse", type="primary", use_container_width=True):
        if not question.strip():
            ...
            if plan["status"] == "ok":
                ...
                render_result(result, plan["question_text"], key_suffix="nl")
    ...
    with st.expander("Prefer to build the analysis manually instead of typing a question?"):
        if not numeric_columns:
            ...
            if result is not None:
                render_result(result, question_text, key_suffix="manual")
else:
