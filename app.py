"""
AI Data Analyst Assistant — Phase 9: professional business reports.

WHAT THIS FILE IS:
The single entry point Streamlit runs. As of Phase 9, Section 8 can
generate a written business summary — but only from results you've
already computed elsewhere in the app this session (tracked in
st.session_state["results_history"]). It never invents new numbers, and
clearly explains when it can't run (no analyses yet, or no AI configured).
"""
    ...
    if st.button("Analyse", type="primary", use_container_width=True):
        if not question.strip():
            ...
            if plan["status"] == "ok":
                ...
                # WHY WE RECORD HISTORY ONLY HERE, NOT INSIDE render_result:
                # This code only runs once, at the moment a NEW result is
                # computed. render_result() also runs on every later rerun
                # (e.g. clicking "Explain with AI") just to redisplay the
                # same result — recording history there would add a
                # duplicate entry every single time you interacted with
    ...
    if st.button("📝 Generate Business Summary", type="primary", use_container_width=True):
        with st.spinner("Writing summary..."):
            try:
                ...
                st.warning(f"Couldn't generate the summary right now ({e}). Please try again.")
